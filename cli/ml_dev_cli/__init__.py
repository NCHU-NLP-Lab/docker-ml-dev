...
def arg_parser_error_handler(message):
    ...
    print("more info: https://github.com/NCHU-NLP-Lab/docker-ml-dev")


parser = argparse.ArgumentParser(
    add_help=True,
    usage="%(prog)s COMMAND\nmore info: https://github.com/NCHU-NLP-Lab/docker-ml-dev",
)
...
def get_image_infos():
    r = requests.get(
        "https://registry.hub.docker.com/v2/repositories/nchunlplab/ml-dev/tags"
    )
...
def list_image_tags():
    ...
    for image in images:
        ...
        images_tags.append("%s/ml-dev:%s" % (last_updater_username, image_name))
    return images_tags
...
def create_container():
    ...
    # docker run options
    docker_run_options = input(
        'any other options for images? e.g "-p 1234:1234 -p 1235:1235":'
    )
